...
parse_args <- function(args) {
  out <- list(mode = "full", config = "config/r1_validation_full.yml", workers = NULL, out = NULL, skip_install = FALSE)
  i <- 1L
  while (i <= length(args)) {
    a <- args[[i]]
    if (grepl("^--mode=", a)) out$mode <- sub("^--mode=", "", a)
    else if (a == "--mode" && i < length(args)) { i <- i + 1L; out$mode <- args[[i]] }
    else if (grepl("^--config=", a)) out$config <- sub("^--config=", "", a)
    else if (a == "--config" && i < length(args)) { i <- i + 1L; out$config <- args[[i]] }
    else if (grepl("^--workers=", a)) out$workers <- sub("^--workers=", "", a)
    else if (a == "--workers" && i < length(args)) { i <- i + 1L; out$workers <- args[[i]] }
    else if (grepl("^--out=", a)) out$out <- sub("^--out=", "", a)
    else if (a == "--out" && i < length(args)) { i <- i + 1L; out$out <- args[[i]] }
    else if (a == "--skip-install") out$skip_install <- TRUE
...
venv_dir <- Sys.getenv("R1_PYTHON_VENV", unset = path.expand(file.path("~", ".cache", "grav_methodsx_r1", "venv")))
...
if (!file.exists(venv_python)) {
  ...
  run_cmd(system_python, c("-m", "venv", shQuote(venv_dir)), "create isolated Python R1 virtual environment")
}
...
venv_check <- paste("import sys;", "assert sys.prefix != sys.base_prefix,", "f'Python is not running inside the R1 virtual environment: prefix={sys.prefix}, base={sys.base_prefix}'")
...
python_check <- paste("import numpy,pandas,scipy,statsmodels,sklearn,yaml;", if (isTRUE(cfg$benchmarks$pcmci)) "import tigramite" else "pass")
py_ok <- system2(python, c("-c", shQuote(python_check)), stdout = FALSE, stderr = FALSE) == 0L
if (!py_ok) {
...
if (file.exists(file.path("tests", "smoke_test.R"))) {
  run_cmd(file.path(R.home("bin"), "Rscript"), c(file.path("tests", "smoke_test.R")), "R1 decision-contract smoke test")
}
...
run_cmd(python, c("python/r1_validation.py", "--config", opt$config, "--mode", "smoke", "--workers", as.character(smoke_workers), "--out", preflight_dir), "R1 end-to-end Python preflight")
run_cmd(file.path(R.home("bin"), "Rscript"), c("R/r1_benchmarks.R", paste0("--config=", opt$config), paste0("--out=", preflight_dir), paste0("--workers=", smoke_workers)), "R1 panel-Granger benchmark preflight")
if (isTRUE(cfg$benchmarks$pcmci)) run_cmd(python, c("python/r1_pcmci_benchmark.py", "--config", opt$config, "--out", preflight_dir, "--workers", as.character(smoke_workers)), "R1 PCMCI benchmark preflight")
run_cmd(python, c("python/r1_finalize.py", "--out", preflight_dir), "R1 aggregation preflight")
log_msg("All R1 preflight stages passed; starting requested full validation.")

py_args <- c("python/r1_validation.py", "--config", opt$config, "--mode", opt$mode, "--workers", as.character(workers), "--out", out_dir)
run_cmd(python, py_args, paste0("R1 Monte Carlo (", opt$mode, ")"))

if (opt$mode %in% c("full", "smoke")) {
  run_cmd(file.path(R.home("bin"), "Rscript"), c("R/r1_benchmarks.R", paste0("--config=", opt$config), paste0("--out=", out_dir), paste0("--workers=", workers)), "Dumitrescu-Hurlin and JKS-HPJ benchmarks")
  if (isTRUE(cfg$benchmarks$pcmci)) run_cmd(python, c("python/r1_pcmci_benchmark.py", "--config", opt$config, "--out", out_dir, "--workers", as.character(workers)), "PCMCI benchmark")
  run_cmd(python, c("python/r1_finalize.py", "--out", out_dir), "reviewer-facing output aggregation")
}
