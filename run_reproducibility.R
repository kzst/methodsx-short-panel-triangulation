...
asset_python <- function() {
  ...
  if (!file.exists(venv_python)) {
    ...
    run_cmd(
      ...
      "create isolated reproducibility Python environment"
    )
  }
  py <- path.expand(venv_python)
  check <- paste(
    "import sys; assert sys.prefix != sys.base_prefix,",
    "f'not in virtual environment: {sys.prefix} == {sys.base_prefix}'"
  )
...
build_assets <- function(py) {
  run_cmd(
    py,
    c("python/build_manuscript_assets.py", "--root", root, "--out", "manuscript_assets"),
...
if (mode %in% c("quick", "full") && file.exists("tests/smoke_test.R")) {
  run_cmd(
    file.path(R.home("bin"), "Rscript"),
    "tests/smoke_test.R",
    "R1 evidence-family contract smoke test"
  )
}
...
if (mode %in% c("quick", "full")) {
  if (file.exists("tests/test_reference_outputs.py")) {
    run_cmd(py, "tests/test_reference_outputs.py", "reference-output contract test")
  }
  if (file.exists("tests/test_r1_contract.py")) {
    run_cmd(py, "tests/test_r1_contract.py", "R1 validation contract test")
  }
}
